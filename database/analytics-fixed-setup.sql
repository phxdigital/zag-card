-- Analytics System - Fixed Setup
-- This script creates the analytics system with proper error handling
...
-- ============================================
-- STEP 4: CREATE MATERIALIZED VIEW (WITH ERROR HANDLING)
-- ============================================

-- Create materialized view with proper error handling
DO $$
BEGIN
    -- Try to create the materialized view
    BEGIN
        ...
        RAISE NOTICE 'Materialized view page_analytics created successfully';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE NOTICE 'Error creating materialized view: %', SQLERRM;
            -- Create an empty materialized view structure
            CREATE MATERIALIZED VIEW page_analytics AS
            SELECT 
                page_id,
                visit_date,
            ...
            FROM (
                SELECT 
                    ...
                    NULL::BIGINT as mobile_visits,
                    NULL::BIGINT as desktop_visits,
                    NULL::BIGINT as tablet_visits
                WHERE FALSE
            ) empty_data;
            
            RAISE NOTICE 'Empty materialized view created as fallback';
    END;
END $$;
...
-- ============================================
-- STEP 8: VERIFICATION (WITH BETTER ERROR HANDLING)
-- ============================================
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_name = 'page_analytics' AND table_schema = 'public'
    ) INTO view_exists;
    ...
    IF view_exists THEN
        RAISE NOTICE '✅ Materialized view page_analytics created successfully';
    ELSE
        RAISE WARNING '⚠️ Materialized view page_analytics was not created - this is OK for now';
    END IF;
    ...
    RAISE NOTICE '🔄 The materialized view will be populated when data is inserted.';
END $$;
