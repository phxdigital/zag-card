-- Analytics System - Clean Setup
-- This script removes any existing analytics objects and creates them fresh
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'page_visits') THEN
        RAISE EXCEPTION 'Table page_visits was not created';
    END IF;
    
    -- Check if materialized view exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'page_analytics') THEN
        RAISE EXCEPTION 'Materialized view page_analytics was not created';
    END IF;
    
    -- Check if functions exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name = 'get_daily_visits') THEN
