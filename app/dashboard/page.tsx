'use client';import React, { useState, useEffect, useRef } from 'react';import QRCode from 'qrcodejs2';import Cropper from 'cropperjs';import 'cropperjs/dist/cropper.css';import {Crop, Wand2, MessageCircle, Instagram, Facebook, Link as LinkIcon,ShoppingCart, Globe, Wifi, DollarSign, BookOpen, MapPin, Phone, Mail, Info,Star, Image as ImageIcon, Video, PlusCircle, Edit, Trash2, Smartphone, CreditCard,User, Circle, Square} from 'lucide-react';import Image from 'next/image';// --- Tipos de Dados ---type CustomLink = {id: number; text: string; url: string; icon: string | null;styleType: 'solid' | 'gradient'; bgColor1: string; bgColor2: string; textColor: string;};type PageConfig = {cardText?: string; isTextEnabled?: boolean; cardBgColor?: string; cardTextColor?: string;cardBackBgColor?: string; logoSize?: number; qrCodeSize?: number; clientLogoBackSize?: number;qrCodePosition?: string; socialLinks?: { [key: string]: string }; customLinks?: CustomLink[];landingPageBgColor?: string; landingPageBgImage?: string | null; landingPageTitleText?: string;landingPageSubtitleText?: string; landingPageLogoShape?: 'circle' | 'square'; landingPageLogoSize?: number;};type IconName =| 'shopping-cart' | 'link' | 'dollar-sign' | 'wifi' | 'globe' | 'book-open'| 'map-pin' | 'phone' | 'mail' | 'info' | 'star' | 'image' | 'video'| 'plus-circle' | 'edit' | 'trash-2' | 'user' | 'circle' | 'square'| 'message-circle' | 'instagram' | 'facebook';// --- Componentes ---const LucideIcon = ({ name, ...props }: { name: IconName; [key: string]: string | number }) => {const icons: { [key in IconName]: React.ElementType } = {'message-circle': MessageCircle, 'instagram': Instagram, 'facebook': Facebook, 'video': Video,'shopping-cart': ShoppingCart, 'link': LinkIcon, 'dollar-sign': DollarSign, 'wifi': Wifi,'globe': Globe, 'book-open': BookOpen, 'map-pin': MapPin, 'phone': Phone, 'mail': Mail,'info': Info, 'star': Star, 'image': ImageIcon, 'plus-circle': PlusCircle,'edit': Edit, 'trash-2': Trash2, 'user': User, 'circle': Circle, 'square': Square};const IconComponent = icons[name];return IconComponent ? <IconComponent {...props} /> : null;};interface LinkEditorModalProps {initialData?: Partial<Omit<CustomLink, 'id'>>; link: CustomLink | null;onSave: (data: Omit<CustomLink, 'id'>) => void; onClose: () => void; icons: string[];}const LinkEditorModal = ({ initialData, link, onSave, onClose, icons }: LinkEditorModalProps) => {const [data, setData] = useState({text: link?.text || initialData?.text || '', url: link?.url || '', icon: link?.icon || null,styleType: link?.styleType || 'solid', bgColor1: link?.bgColor1 || '#1e293b',bgColor2: link?.bgColor2 || '#475569', textColor: link?.textColor || '#ffffff',});const handleSubmit = () => {
    if (!data.text || !data.url) { alert('Texto e URL são obrigatórios.'); return; }
    onSave(data);
};
...
useEffect(() => {
    if (isClient && qrcodePreviewRef.current && subdomain) {
...
const openLinkEditor = (link: CustomLink | null = null, defaults: Partial<Omit<CustomLink, 'id'>> = {}) => {
    setEditingLink(link);
    const initialData: Partial<Omit<CustomLink, 'id'>> = link ? { ...link } : {
        text: defaults.text || '', url: defaults.url || '', icon: defaults.icon || null,
        styleType: defaults.styleType || 'solid', bgColor1: defaults.bgColor1 || '#1e293b',
        bgColor2: defaults.bgColor2 || '#475569', textColor: defaults.textColor || '#ffffff',
    };
    (window as any)._linkEditorData = initialData;
    setShowLinkEditorModal(true);
};

const saveCustomLink = (linkData: Omit<CustomLink, 'id'>) => {
    if (editingLink) {
        setConfig(prev => ({...prev, customLinks: prev.customLinks?.map(l => l.id === editingLink.id ? { ...l, ...linkData } : l)}));
    } else {
        if ((config.customLinks?.length || 0) >= 4) {
            alert('Você pode adicionar no máximo 4 links personalizados.'); return;
        }
        setConfig(prev => ({...prev, customLinks: [...(prev.customLinks || []), { ...linkData, id: Date.now() }]}));
    }
    ...
    <>
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
            ...
            <div className="flex items-center justify-center space-x-4 md:space-x-8 mb-8">
                <div className={`flex items-center space-x-2 border-b-4 pb-2 transition-all duration-300 ${activeStep === 1 ? 'border-amber-500 text-amber-600' : 'border-green-500 text-green-600'}`}>
                ...
                <div className={`flex items-center space-x-2 border-b-4 pb-2 transition-all duration-300 ${activeStep === 2 ? 'border-amber-500 text-amber-600' : 'border-transparent text-slate-500'}`}>
            ...
            <main id="workspace">
                    ...
                    <div>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="bg-white p-6 rounded-xl shadow-sm border">
                                ...
                                <div style={{ backgroundColor: config.cardBgColor }} className="card-preview mx-auto rounded-xl shadow-lg flex flex-col items-center justify-center p-4 border">
                                    {logoDataUrl ? (
                                        <Image src={logoDataUrl} alt="Logo Preview" width={200} height={200} className="object-contain mb-2" style={{ width: `${config.logoSize}%`, height: 'auto', maxHeight: '80%' }}/>
                                    ) : (
                                    ...
                                    {config.isTextEnabled && (
                                        <p style={{color: config.cardTextColor}} className="font-semibold text-lg text-center">{config.cardText || 'Seu Nome'}</p>
                                    )}
                                </div>
                                <div className="mt-6 space-y-4 max-w-sm mx-auto">
                                    ...
                                    <div>
                                     ...
                                     <div className={`grid grid-cols-2 gap-2 text-sm ${!logoDataUrl ? 'hidden' : ''}`}>
                                    ...
                                    <div>
                                        ...
                                        <input type="range" min="30" max="90" value={config.logoSize} onChange={(e) => handleConfigChange('logoSize', Number(e.target.value))} className="w-full"/>
                                    </div>
                                    <div>
                                        <div className="flex items-center mb-2">
                                            <input type="checkbox" checked={!!config.isTextEnabled} onChange={(e) => handleConfigChange('isTextEnabled', e.target.checked)} className="h-4 w-4 text-amber-600"/>
                                            <label className="ml-2 block text-sm font-medium text-slate-700">Adicionar texto?</label>
                                        </div>
                                        <input type="text" placeholder="Seu Nome ou Empresa" value={config.cardText || ''} onChange={(e) => handleConfigChange('cardText', e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-md disabled:bg-slate-100" disabled={!config.isTextEnabled}/>
                                    </div>
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border">
                                ...
                                <div className="mt-6 space-y-4 max-w-sm mx-auto">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex">
                                            <input type="text" placeholder="sua-empresa" value={subdomain} onChange={(e) => setSubdomain(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ''))} className="w-full px-3 py-2 border border-slate-300 rounded-l-md"/>
                                            <span className="inline-flex items-center px-3 rounded-r-md border border-l-0 border-slate-300 bg-slate-50 text-slate-500 text-sm">.meuzag.com</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="mt-8 flex justify-center">
                            <button onClick={handleNextStep} className="w-full max-w-md bg-slate-800 text-white font-bold py-3 px-4 rounded-lg hover:bg-slate-900">Próximo Passo</button>
                        </div>
                    </div>
                )}
                ...
                {activeStep === 2 && (
                    <div>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="bg-white p-6 rounded-xl shadow-sm border overflow-y-auto" style={{maxHeight: '85vh'}}>
                                        ...
                                        <div>
                                            ...
                                            {showEmojiPicker && (
                                                <div className="grid grid-cols-8 gap-1 p-2 bg-white border rounded-lg shadow-lg mt-2 absolute z-10">
                                                    {commonEmojis.map(emoji => <button key={emoji} onClick={() => { handleConfigChange('landingPageSubtitleText', (config.landingPageSubtitleText || '') + emoji); setShowEmojiPicker(false); }} className="emoji-picker-item text-xl p-1">{emoji}</button>)}
                                                </div>
                                            )}
                                        </div>
                                        <div className="pt-2">
                                         ...
                                         <div>
                                            <label className="block text-sm font-medium text-slate-700 mb-1">Tamanho da Logo ({config.landingPageLogoSize}px)</label>
                                            <input type="range" min="48" max="128" value={config.landingPageLogoSize || 96} onChange={(e) => handleConfigChange('landingPageLogoSize', Number(e.target.value))} className="w-full"/>
                                        </div>
                                    </div>
                                </fieldset>
                                    
                                    <fieldset className="border-t pt-4">
                                        <legend className="text-lg font-semibold text-slate-800 -mt-7 px-2 bg-white">Links Personalizados</legend>
                                        <div className="space-y-4 mt-4">
                                            <div>
                            ...
                            <div className="phone-preview bg-white flex flex-col overflow-hidden">
                                ...
                                <div id="phone-content-preview" style={{backgroundColor: config.landingPageBgColor, backgroundImage: config.landingPageBgImage ? `url('${config.landingPageBgImage}')` : 'none', backgroundSize: 'cover', backgroundPosition: 'center' }} className="flex-grow overflow-y-auto preview-content p-4">
                                    <div className="flex flex-col items-center text-center">
                                        {logoDataUrl && <Image src={logoDataUrl} alt="Logo Preview" width={config.landingPageLogoSize} height={config.landingPageLogoSize} className={`object-cover mx-auto mb-4 shadow-md ${config.landingPageLogoShape === 'circle' ? 'rounded-full' : 'rounded-2xl'}`} />}
                     ...
                     <div className="mt-8 flex justify-center">
                         <button className="w-full max-w-md bg-green-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-green-700 disabled:bg-green-400">
                             Salvar e Publicar
                         </button>
                    </div>
                </div>
