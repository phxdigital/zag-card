            ...
            {showLinkEditorModal && (
                <LinkEditorModal 
                    initialData={typeof window !== 'undefined' ? (window as { _linkEditorData?: LinkEditorData })._linkEditorData : undefined} 
                    link={editingLink} 
                    ...
                    <div className="grid grid-cols-6 sm:grid-cols-8 gap-2 max-h-32 overflow-y-auto p-2 bg-slate-50 rounded-md">
                        {icons.map(icon => (
                            <div 
                                ...
                                className={`icon-picker-item p-2 border rounded-md flex items-center justify-center cursor-pointer hover:bg-amber-100 ${data.icon === icon ? 'bg-amber-200 border-amber-400' : 'border-slate-300'}`}
                            >
                    ...
                    <nav className="-mb-px flex space-x-4" aria-label="Tabs">
                        <button 
                            ...
                            className={`tab-btn whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm ${
                                isSimple ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                            }`}
                        >
