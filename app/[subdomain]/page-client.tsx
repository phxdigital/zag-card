'use client';import {MessageCircle, Instagram, Facebook, Link as LinkIcon,ShoppingCart, Globe, Wifi, DollarSign, BookOpen, MapPin, Phone, Mail, Info,Star, Image as ImageIcon, Video} from 'lucide-react';import Image from 'next/image';// --- Tipos de Dados ---export type SocialLinks = { [key: string]: string };export type CustomLink = {id: number; text: string; url: string; icon: string | null;styleType: 'solid' | 'gradient'; bgColor1: string; bgColor2: string; textColor: string;};export type PageConfig = {cardText?: string;landingPageBgColor?: string;landingPageBgImage?: string | null;landingPageTitleText?: string;landingPageSubtitleText?: string;landingPageLogoShape?: 'circle' | 'square';landingPageLogoSize?: number;socialLinks?: SocialLinks;customLinks?: CustomLink[];};interface PageClientProps {config: PageConfig;logoUrl: string;}type SocialMediaConfig = {[key: string]: {icon: string;baseUrl: string;}}const socialMediaConfig: SocialMediaConfig = {whatsapp: { icon: 'message-circle', baseUrl: 'https://wa.me/' },instagram: { icon: 'instagram', baseUrl: 'https://instagram.com/' },facebook: { icon: 'facebook', baseUrl: 'https://facebook.com/' },tiktok: { icon: 'music-2', baseUrl: 'https://tiktok.com/' },};const LucideIcon = ({ name, ...props }: { name: string, [key: string]: any }) => {const icons: { [key: string]: React.ElementType } = {'message-circle': MessageCircle, 'instagram': Instagram, 'facebook': Facebook,'shopping-cart': ShoppingCart, 'link': LinkIcon, 'dollar-sign': DollarSign,'wifi': Wifi, 'globe': Globe, 'book-open': BookOpen, 'map-pin': MapPin,'phone': Phone, 'mail': Mail, 'info': Info, 'star': Star, 'image': ImageIcon, 'video': Video};const IconComponent = icons[name];return IconComponent ? <IconComponent {...props} /> : null;};export default function PageClient({ config, logoUrl }: PageClientProps) {const handleLinkClick = (url: string) => {
