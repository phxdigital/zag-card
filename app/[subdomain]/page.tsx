...
export default function DashboardPage() {
    const [config, setConfig] = useState<PageConfig>({
        ...
        landingPageTitleText: 'Seu Título Aqui',
        landingPageSubtitleText: '',
        landingPageLogoShape: 'circle',
        landingPageLogoSize: 96, // h-24 in Tailwind
    });
    
    const [activeStep, setActiveStep] = useState(1);
    const [subdomain, setSubdomain] = useState('sua-empresa');
    ...
    useEffect(() => {
        if (qrcodePreviewRef.current) {
            ...
            const url = subdomain ? `https://${subdomain}.meuzag.com` : 'https://meuzag.com';
        ...
        if (file) {
            ...
            reader.onload = async (event) => {
                ...
                try {
                    ...
                    const palette = await colorThief.getPalette(dataUrl, 5);
                    // Lógica para sugerir paleta aqui
                } catch (error) {
                    console.error("Erro ao obter paleta:", error);
                }
            };
    ...
    const handleNextStep = () => {
        ...
        setConfig(prev => ({ ...prev, landingPageTitleText: prev.cardText }));
        setActiveStep(2);
    };
    ...
    return (
        <>
            ...
            <div className="max-w-7xl mx-auto">
                ...
                {/* Workspace */}
                <main id="workspace">
                    {/* ETAPA 1: DESIGN DO CARTÃO */}
                    <div className={activeStep === 1 ? '' : 'hidden'}>
                         <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                             {/* Coluna da Frente */}
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                                <p className="text-center font-semibold mb-4">Frente</p>
                                <div id="card-front-preview" style={{ backgroundColor: config.cardBgColor }} className="card-preview mx-auto rounded-xl shadow-lg flex flex-col items-center justify-center p-4 transition-colors duration-300 border">
                                    ...
                                    {config.isTextEnabled && <p id="card-text-preview" style={{color: config.cardTextColor}} className="font-semibold text-lg text-center">{config.cardText}</p>}
                                </div>
                                <div className="mt-6 space-y-4 max-w-sm mx-auto">
                                    ...
                                    <div className={`grid grid-cols-2 gap-2 text-sm ${!logoDataUrl || logoDataUrl.startsWith('https://placehold.co') ? 'hidden' : ''}`}>
                                        ...
                                        <a href="https://www.remove.bg/upload" target="_blank" className="w-full flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 p-2 rounded-md"><Wand2 className="w-4 h-4"/> Remover Fundo</a>
                                    </div>
                                    <div>
                            ...
                            {/* Coluna do Verso */}
                             <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                                ...
                                <div id="card-back-preview" style={{backgroundColor: config.cardBackBgColor}} className="card-preview mx-auto rounded-xl shadow-lg p-4 border relative overflow-hidden">
                                    {logoDataUrl && <Image src={logoDataUrl} alt="Logo Verso" width={150} height={150} className="object-contain absolute transition-all duration-300" style={{ width: `${config.clientLogoBackSize}%`, top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}/>}
                                    <div id="qrcode-wrapper" className={`absolute inset-0 p-4 flex items-center ${config.qrCodePosition}`}>
                                        <div ref={qrcodePreviewRef} className="bg-white p-1 rounded-md aspect-square" style={{ width: `${config.qrCodeSize}%` }}></div>
                                    </div>
                                    <Image src="/logo.png" alt="Logo Zag Card" width={100} height={30} className="absolute bottom-3 right-3 h-5 object-contain"/>
                                </div>
                                <div className="mt-6 space-y-4 max-w-sm mx-auto">
                           ...
                           <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 overflow-y-auto" style={{maxHeight: '85vh'}}>
                               <div className="space-y-6">
                                    <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                                        <p className="text-sm font-medium text-green-800">Seu design foi enviado! Agora, personalize sua página.</p>
                                    </div>
                                    <fieldset className="border-t pt-4">
                                        ...
                                        <div className="space-y-3 mt-4">
                                            ...
                                            <div className="pt-2">
                                             ...
                                             <div>
                                                <label htmlFor="landing-logo-size-slider" className="block text-sm font-medium text-slate-700 mb-1">Tamanho da Logo</label>
                                        ...
                                        <div className="space-y-4 mt-4">
                                            <div>
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    <button onClick={() => {openLinkEditor();}} data-text="Catálogo" className="px-3 py-1 bg-slate-200 text-slate-700 rounded-full text-sm hover:bg-slate-300">Catálogo</button>
                                                    <button onClick={() => {openLinkEditor();}} data-text="Website" className="px-3 py-1 bg-slate-200 text-slate-700 rounded-full text-sm hover:bg-slate-300">Website</button>
                                ...
                                <div className="phone-preview bg-white flex flex-col overflow-hidden">
                                    ...
                                    <div id="phone-content-preview" style={{backgroundColor: config.landingPageBgColor, backgroundImage: config.landingPageBgImage ? `url('${config.landingPageBgImage}')` : 'none', backgroundSize: 'cover', backgroundPosition: 'center' }} className="flex-grow overflow-y-auto preview-content p-4">
                                        <div className="flex flex-col items-center text-center">
                                            <Image src={logoDataUrl || ''} alt="Logo Preview" width={128} height={128} className={`object-contain mb-4 shadow-md ${config.landingPageLogoShape === 'circle' ? 'rounded-full' : 'rounded-2xl'}`} style={{width: `${config.landingPageLogoSize}px`, height: `${config.landingPageLogoSize}px`}} />
                                            <h1 className="text-2xl font-bold text-slate-800">{config.landingPageTitleText}</h1>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg space-y-4">
                        ...
                        <div className="w-full h-64 bg-slate-100">
                            <img ref={cropperImageRef} src={logoDataUrl || ''} className="max-w-full"/>
                        </div>
            ...
            {showLinkEditorModal && <LinkEditorModal link={editingLink} onSave={saveCustomLink} onClose={() => setShowLinkEditorModal(false)} icons={availableIcons} />}
...
const LinkEditorModal = ({ link, onSave, onClose, icons }: { link: CustomLink | null, onSave: (data: Omit<CustomLink, 'id'>) => void, onClose: () => void, icons: string[] }) => {
...
const PixConfigModal = ({ onClose, onSave }: { onClose: () => void, onSave: (data: Omit<CustomLink, 'id'>) => void }) => {
    ...
    const handleSave = () => {
        ...
        if (isSimple) {
            ...
            linkData = { text: 'Pagar com PIX', url: `pix:${pixCode}` };
        }
            ...
            <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg space-y-4">
                ...
                <div className={`space-y-3 ${!isSimple ? '' : 'hidden'}`}>
                    <p className="text-sm text-slate-600 mb-3">Cole o código "PIX Copia e Cola" gerado pelo seu banco.</p>
                     <div>
                        <label htmlFor="pix-code-advanced" className="block text-sm font-medium text-slate-700 mb-1">Código PIX</label>
